...
import { ArrowRight, Sparkles, Award, Filter, Search, MapPin, Briefcase } from "lucide-react";
...
export default function CaseStudiesPage() {
  ...
  const [visibleCards, setVisibleCards] = useState<boolean[]>(new Array(caseStudiesData.length).fill(false));
            ...
            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
              {filteredCaseStudies.map((study, index) => (
                <Link
                  ...
                  className={`case-study-card group block transition-all duration-700 ${visibleCards[index] ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'
                    }`}
                  data-index={index}
                >
