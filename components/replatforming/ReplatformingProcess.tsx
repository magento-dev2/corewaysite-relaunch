...
import { Search, Settings, Database, TestTube, Rocket } from 'lucide-react';
...
interface ReplatformingProcessProps {
  title: string;
  description: string;
  steps: Step[];
}
...
export default function ReplatformingProcess({ title, description, steps }: ReplatformingProcessProps) {
            ...
            {steps.map((step, index) => {
              ...
              return (
                ...
                >
                  ...
                  >
                    <div className={`relative bg-white/5 backdrop-blur-sm border rounded-2xl p-8 transition-all duration-500 group cursor-pointer ${isActive
                      ...
                      <div className="flex items-center space-x-4 mb-4 lg:hidden">
                        <div className={`w-12 h-12 rounded-full flex items-center justify-center transition-all duration-500 ${isActive
                        ...
                        <span className={`text-4xl font-bold transition-colors ${isActive ? 'text-purple-400' : 'text-purple-500/50'
                          }`}>
                          {step.number}
                        </span>
                      </div>

                      <h3 className={`text-2xl font-bold mb-3 transition-colors duration-300 ${isActive ? 'text-purple-300' : 'text-white'
                        }`}>
                        {step.title}
                      </h3>
                  ...
                  <div className={`hidden lg:flex absolute left-1/2 transform -translate-x-1/2 w-16 h-16 rounded-full items-center justify-center z-10 border-4 border-[#0E0918] transition-all duration-500 ${isActive
