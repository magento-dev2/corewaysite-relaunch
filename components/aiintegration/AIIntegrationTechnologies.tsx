...
import { Sparkles, Cloud, Box, Brain, Link2, Zap } from 'lucide-react';
...
interface AIIntegrationTechnologiesProps {
  title: string;
  description: string;
  items: Platform[];
}
...
export default function AIIntegrationTechnologies({ title, description, items }: AIIntegrationTechnologiesProps) {
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-6">
          {items.map((platform, index) => {
            const Icon = platformIcons[platform.id] || Sparkles;
        ...
        <div className="mt-16 text-center">
          <p className="text-gray-400 text-sm">
            Seamless integration with industry-leading AI platforms
          </p>
        </div>
      </div>
    </section>
