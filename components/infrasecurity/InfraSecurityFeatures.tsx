...
import { AlertTriangle, Shield, Zap, FileCheck, Bug, Lock } from 'lucide-react';
...
interface InfraSecurityFeaturesProps {
  title: string;
  items: Feature[];
}

const iconMap: Record<string, any> = {
  'threat-detection': AlertTriangle,
  'zero-trust': Shield,
  'automated-response': Zap,
  'compliance': FileCheck,
  'vulnerability': Bug,
  'encryption': Lock,
};

export default function InfraSecurityFeatures({ title, items }: InfraSecurityFeaturesProps) {
    ...
    >
      <div className="absolute inset-0 overflow-hidden">
        <div className="absolute inset-0">
          {[...Array(30)].map((_, i) => (
            <div
              key={i}
              className="absolute animate-security-particle"
              style={{
                left: `${5 + (i % 6) * 16}%`,
                top: `${-10 + (i % 5) * 25}%`,
            ...
            >
              <Shield className="text-purple-500/20" size={16} />
            </div>
          ))}
        </div>
      ...
      <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center mb-16">
          ...
          <p className="text-xl text-gray-300 max-w-2xl mx-auto font-mono text-sm">
            $ secure-infrastructure --mode=zero-trust --ai-enabled
          </p>
        </div>
                ...
                {isHovered && (
                  <>
                    ...
                    <div className="absolute inset-0 rounded-2xl overflow-hidden">
                      <div className="absolute inset-0 flex flex-col space-y-1 p-2">
                        {[...Array(8)].map((_, i) => (
                          <div
                            key={i}
                            className="h-1 bg-purple-500/20 rounded-full animate-security-scan"
                            style={{
      ...
      <style>{`
        ...
        .animate-security-particle {
          animation: security-particle linear infinite;
        }
