...
interface CustomAPIAutomationProps {
  title: string;
  description: string;
  features: string[];
}

export default function CustomAPIAutomation({ title, description, features }: CustomAPIAutomationProps) {
              ...
              <div className="aspect-video bg-gradient-to-br from-purple-500/20 to-violet-500/20 rounded-xl flex items-center justify-center relative overflow-hidden">
                <div className="absolute inset-0">
                  {[...Array(6)].map((_, i) => (
                    <div
                      key={i}
                      className="absolute bg-purple-500/20 rounded-lg animate-pulse"
                      style={{
                        top: `${15 + i * 12}%`,
                        left: `${10 + i * 15}%`,
                        width: `${40 + i * 8}px`,
                ...
                <div className="relative text-center z-10">
                  <Lock className="text-purple-500 mx-auto mb-4 animate-pulse" size={64} />
                  <p className="text-white font-medium text-lg">Enterprise API Security</p>
