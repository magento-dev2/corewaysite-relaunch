...
import { Workflow, Database, Radio, Shield, TrendingUp, Zap, Headphones, FileText, Cloud, Activity, Cpu } from 'lucide-react';
...
const iconMap: Record<string, any> = {
  ...
  'dbdashbot': Database,
  'rag-chatbot': Radio,
  'ai-chat-pdf': FileText,
  'image-recognition': Cpu,
  'ugs-ads': TrendingUp,
  'replatforming': Zap
};

export default function Solutions({ title, items }: SolutionsProps) {
      ...
      <div className="absolute inset-0 overflow-hidden">
        <div className="absolute inset-0">
          {[...Array(50)].map((_, i) => (
            ...
            >
              {['AI-AGENT', 'AUTOMATE', 'WORKFLOW', 'GPT-4', 'LLM', 'API', 'TASK'][Math.floor(Math.random() * 7)]}
            </div>
          ))}
        </div>
      ...
      <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center mb-16">
          ...
          <p className="text-lg text-gray-300 max-w-2xl mx-auto font-mono text-sm">
            {'>'} Strategic AI Solutions for Modern Enterprise Automation
          </p>
        </div>
