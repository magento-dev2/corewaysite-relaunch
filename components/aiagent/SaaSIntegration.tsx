...
export default function SaaSIntegration({ title, subtitle, items, img }: SaaSIntegrationProps) {
        ...
        <div className="grid lg:grid-cols-2 gap-12 items-center">
          ...
          >
            <div className="inline-flex items-center space-x-2 bg-white/5 backdrop-blur-sm border border-white/10 rounded-full px-4 py-2 mb-6">
              ...
              <span className="text-gray-300 text-sm font-mono">{'<AI/>'} SaaS Integration</span>
            </div>
          ...
          >
            <div className="relative rounded-2xl overflow-hidden bg-white/5 backdrop-blur-sm border border-white/10 p-8">
              {img ? (
                <img src={img} alt="Integration" className="w-full h-auto rounded-xl shadow-2xl" />
              ) : (
                  ...
                  <div className="absolute inset-0 p-4">
                    <div className="space-y-2">
                      {dataRows.map((row) => (
                        <div
                          key={row.id}
                          className={`h-6 rounded transition-all duration-500 ${row.active
                        ...
                        >
                          <div className="h-full flex items-center px-3 space-x-2 font-mono text-xs">
                            ...
                            <span className="text-purple-300">{row.active ? 'CONNECTED' : 'SYNCING'}</span>
                          </div>
                        </div>
                      ))}
                    </div>
