...
import { Brain, Cloud, Shield, AlertTriangle, Activity, Lock } from 'lucide-react';
...
interface AISecurityTechnologiesProps {
  title: string;
  description: string;
  items: Platform[];
}
...
const platformIcons: Record<string, any> = {
  ...
  crowdstrike: AlertTriangle,
  splunk: Activity,
  'palo-alto': Lock,
};

export default function AISecurityTechnologies({ title, description, items }: AISecurityTechnologiesProps) {
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-6">
          {items.map((platform, index) => {
            const Icon = platformIcons[platform.id] || Brain;
        ...
        <div className="mt-16 text-center">
          <p className="text-gray-400 text-sm">
            Industry-leading AI and security platforms for enterprise protection
          </p>
        </div>
      </div>
    </section>
